...
static AST* parse_term(Parser* ps){
    ...
    if (tok.type == TOK_ELSE){
        ...
        else_AST->type = AST_IF;
        eat(ps, TOK_ELSE);
        eat(ps, TOK_LPAREN);
        else_AST->cond = parse_expr(ps);
        eat(ps, TOK_RPAREN);
        eat(ps, TOK_LCURLY);
    ...
    // I think that we are creating many AST_ID for the same AST_ID
    if (tok.type == TOK_ID){
        eat(ps, TOK_ID);
...
int eval_ast_assignment(const AST *node) {
    switch (node->type) {
        ...
        case AST_IF: {
            int cond_value = eval_ast_assignment(node->cond);
            if (cond_value) {
                return eval_ast_assignment(node->left);
            } else return eval_ast_assignment(node->right);
         }
        case AST_BINOP: {

...
static void eat(Parser *ps, TokenType expect) {
	if (ps->current.type == expect) {
		...
		printf("EAT: expecting %d, got %d (%s)\n", expect, ps->current.type, ps->current.lexeme);
	} else {
...
static int is_returnType(Parser* ps){
	if (ps->current.type == TOK_INT_VAR||ps->current.type == TOK_VOID|| ps->current.type == TOK_FLOAT_VAR || ps->current.type == TOK_DBL) {
		return 1;
	} else return 0;
}
...
AST* parse_fn(Parser *ps) {
	...
	eat(ps, TOK_LPAREN);  // consume '('
	// Parse parameters: "int a, int b"
	while (ps->current.type != TOK_RPAREN && ps->current.type != TOK_EOF) {
		printf(">> Echo (parse_fn): Parsing parameter, current token: %s\n", ps->current.lexeme); // DEBUG:
	...
	eat(ps, TOK_RPAREN);  // consume ')'
	printf(">> Echo (parse_fn): Parameter count: %d\n", fn->stmnt_cnt);   // DEBUG: Check how many parameters were added

	// Parse function BODYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY
	if (ps->current.type != TOK_LCURLY) {
		syntax_error("expected '{' to start function body", ps->current);
		return fn;
	}
	...
	while (ps->current.type != TOK_RCURLY && ps->current.type != TOK_EOF) {
		...
		if (!stmt) {
			// Error recovery
			printf(">> Echo (parse_fn): Failed to parse statement! \n");
			while (ps->current.type != TOK_SEMI &&
			       ps->current.type != TOK_RCURLY &&
			       ps->current.type != TOK_EOF) {
				advance(ps);
			}
			if (ps->current.type == TOK_SEMI) eat(ps, TOK_SEMI);
			continue;
		}

		// confirm statement was added
		printf(">> Echo (parse_fn): Adding statement, type: %d\n", stmt->type);
		if (fn->stmnt_cnt >= FN_STMTS_CAP) {
			syntax_error("too many statements in function", ps->current);
			break;
		}
	...
	// DEBUG: Final count
	printf(">> Echo (parse_fn): Total nodes (params + statements): %d\n", fn->stmnt_cnt);

	if (ps->current.type == TOK_RCURLY) {
		eat(ps, TOK_RCURLY);
	} else {
		syntax_error("Syntax Error >> parse_fn : expected '}' before end of file", ps->current);
	}
	return fn;
}
...
// PARSE DEFINITION #########################################################################################################
void error_at(Parser* ps, Token* tok, const char* msg) {
...
static AST* parse_definition(Parser* ps){
	Token type_tok = ps->current;  // Save the type token (TOK_INT_VAR, etc.)

	if (ps->current.type != TOK_INT_VAR &&
	    ps->current.type != TOK_FLOAT_VAR &&
	    ps->current.type != TOK_VOID) {
		error_at(ps, &ps->current, "expected type specifier ('int' or 'float')");
		return NULL;
	}
	...
	node->type = AST_DECLARATION;  // ← ALWAYS DECLARATION!
	node->op = type_tok;            // Store the type (TOK_INT_VAR or TOK_FLOAT_VAR)
	node->left = make_id(name);

	// Check for initializer
	if (ps->current.type == TOK_ASSIGN) {
		eat(ps, TOK_ASSIGN);
		node->right = parse_expr(ps); // Store initializer expression
	} else {
		node->right = NULL; // No initializer
	}
...
static AST* parse_term(Parser* ps){
	...
	if (tok.type == TOK_FOR) {
		...
		eat(ps, TOK_LCURLY);
		while (ps->current.type != TOK_RCURLY && ps->current.type != TOK_EOF) {
			printf(">> Echo (parse_for): Parsing statement, current token: %s\n", ps->current.lexeme); // print  statement parsing
			AST *stmt = parse_statement(ps);
			if (!stmt) {
				// Error recovery
				printf(">> Echo (parse_fn): Failed to parse statement! \n");
			...
			// confirm statement was added
			printf(">> Echo (parse_fn): Adding statement, type: %d\n", stmt->type);
			if (for_AST->stmnt_cnt >= FN_STMTS_CAP) {
				syntax_error("too many statements in for body", ps->current);
				break;
			}
			for_AST->stmnts[for_AST->stmnt_cnt++] = stmt;
		}

		// DEBUG: Final count
		printf(">> Echo (parse_for): Total nodes (params + statements): %d\n", for_AST->stmnt_cnt);

		if (ps->current.type == TOK_RCURLY) {
			eat(ps, TOK_RCURLY);
		} else {
			syntax_error("Syntax Error >> parse_for : expected '}' before end of file", ps->current);
		}
		return for_AST;
	}
	...
	if (tok.type == TOK_FLOAT) {
		double v = strtod((tok.lexeme) ? tok.lexeme : "0", NULL);
		eat(ps, TOK_FLOAT);
		AST *num = calloc(1, sizeof *num);
		num->type = AST_NUM;
		num->value = (float)v;
		return num;
	}
...
double eval_binary( TokenType op, double left, double right) {
	switch (op) {
	// binary arithmertic
	...
	// bitwise
	case TOK_BITWISE_AND:  return (int)left &  (int)right;
	case TOK_BITWISE_OR:   return (int)left |  (int)right;
	case TOK_BITWISE_XOR:  return (int)left ^  (int)right;
	...
	//shifting
	case TOK_SHIFT_LEFT:   return (int)left << (int)right;
	case TOK_SHIFT_RIGHT:  return (int)left >> (int)right;
	default:
		fprintf(stderr, "Runtime error: unknown operator\n");
		exit(EXIT_FAILURE);
	}
}
// logic for variables holder****************************************************************************
static int* slot_for(const char* name) {
	for (int i=0; i<128; i++)
	{if (vars[i].used && strcmp(vars[i].name,name)==0) return &vars[i].num_val.i; }
	for (int i=0; i<128; i++)
	{if (!vars[i].used) { vars[i].used=1;
		              strncpy(vars[i].name,name,63); vars[i].name[63]=0; return &vars[i].num_val.i; } }
	fprintf(stderr,"Symbol table full\n"); exit(EXIT_FAILURE);
}

// logic for number type -> map to type float or int*********************************************************
static Var* slot_for_entry(const char* name) {
	for (int i = 0; i < 128; i++) {
	...
	for (int i = 0; i < 128; i++) {
		if (!vars[i].used) {
			vars[i].used = 1;
			strncpy(vars[i].name, name, 63);
			vars[i].name[63] = '\0';
			//vars[i].type = 0;        // “unset” until declared
...
// logic for var type -> map to type float or int DECLARATION**********************************************
static double get_var(const char* name) {
	for (int i = 0; i < 128; i++) {
		if (vars[i].used && strcmp(vars[i].name, name) == 0) {
			if (vars[i].op.type == TOK_FLOAT_VAR) {
				printf(">> Echo (get_var) : name: '%s', value:'%f' \n",name, (double)vars[i].num_val.f);
				return (double)vars[i].num_val.f;
			}
			else if (vars[i].op.type == TOK_INT_VAR) {
				printf(">> Echo (get_var) : name: '%s', value:'%f' \n",name, (double)vars[i].num_val.i);
				return (double)vars[i].num_val.i;
			}
		}
	}
	//fprintf(stderr, "Undefined variable '%s'\n", name);
	//exit(EXIT_FAILURE);
	fprintf(stderr, ">> Echo (get_var) : Undefined variable '%s'\n", name); //debug right here
	fprintf(stderr, ">> Echo (get_var) : Currently registered variables:\n");
	for (int i = 0; i < 128; i++) {
		if (vars[i].used) {
			fprintf(stderr, "  [%d] name='%s', type=%d\n", i, vars[i].name, vars[i].op.type);
		}
	}
	exit(EXIT_FAILURE);
}

// logic for for loop DECLARATION*************************************************************************
static int truthy(double v) {
	return v != 0.0;
}                                                    // return true or false both float number as well
// evaluate statement list in for
static void eval_stmt_list(AST* first) {
	for (AST* it = first; it; it = it->right) {
		(void)eval_ast_assignment(it);
	}
}
// eval the block of for loop
static double eval_block(AST* blk) {
	// Execute linear list of statements attached via right links in blk->left
	eval_stmt_list(blk->left);
	return 0.0;
}
// evaluate for, init for for
static double eval_for(AST* n) {
	if (n->init) (void)eval_ast_assignment(n->init);
	double last = 0.0;
	for (;;) {
		double c = 1.0;
		if (n->cond) c = eval_ast_assignment(n->cond);
		if (!truthy(c)) break;
		// body
		if (n->right) {
		...
		// step
		if (n->left) (void)eval_ast_assignment(n->left);
	}
	return last;
}
...
double eval_ast_decl(AST* n) {
	...
	Var* v = slot_for_entry(name);
	v->used = 1;
	if (n->op.type == TOK_INT || n->op.type == TOK_INT_VAR) {
		v->op.type = TOK_INT_VAR;
	} else if (n->op.type == TOK_FLOAT || n->op.type == TOK_FLOAT_VAR) {
		v->op.type = TOK_FLOAT_VAR;
	} else {
		v->op.type = n->op.type; // Fallback
	}
	// Evaluate the initializer if it exists!
	if (n->right) {      // n->right should be the "a + b" part
		double init_val = eval_ast_assignment(n->right);
		if (v->op.type == TOK_FLOAT_VAR) {
		...
		if (v->op.type == TOK_FLOAT_VAR) v->num_val.f = 0.0f;
		else v->num_val.i = 0;
	}

	return 0.0;
}
// IMPORTANT LOGIC!!!! evaluate the whole AST- SEMANTIC LOGIC **********************************************************
double eval_ast_assignment( AST *node) {
	switch (node->type)
	{
	case AST_NUM:           return node->value;
	case AST_RETURN:        return node->value;
	...
	case AST_FOR:           return eval_for(node);
	...
	case AST_BINOP: {
		double left = eval_ast_assignment(node->left);
		double right = eval_ast_assignment(node->right);
	...
	fprintf(stderr,">> Echo (eval_ast_assignment): Runtime error->bad AST node\n"); exit(EXIT_FAILURE);
}
// IMPORTANT LOGIC!!!! AST DRAWING**********************************************************
...
// Better tree printing implementation! IMPORTANT LOGIC!!! -----------------------------------------------------------------
static ASTWrapper *build_wrapper_tree(const AST *node) {
	...
	ASTWrapper *wrapper = calloc(1, sizeof(ASTWrapper));
	wrapper->node = node;
	// Format label based on node type FIRST
	switch(node->type) {
	case AST_NUM:           sprintf(wrapper->label, "NUM(%.2f)", node->value); break;
	case AST_DECLARATION:   sprintf(wrapper->label, "DECL(%s)",
		                        (node->op.type == TOK_FLOAT_VAR) ? "float" : "int"); break;
	case AST_FUNC:          sprintf(wrapper->label, "FUNC(%s)", node->name); break;
	case AST_RETURN:
		if (node->stmnt_cnt > 0 && node->stmnts[0]) {
			sprintf(wrapper->label, "RETURN");
		} else {
			sprintf(wrapper->label, "RETURN(void)");
		}
		break;
	case AST_ID:            sprintf(wrapper->label, "ID(%s)", node->name); break;
	case AST_IF:            sprintf(wrapper->label, "IF"); break;          // ← Removed (?)
	case AST_WHILE:         sprintf(wrapper->label, "WHILE"); break;          // ← Don't evaluate!
	case AST_ELIF:          sprintf(wrapper->label, "ELSE IF"); break;          // ← Don't evaluate!
	case AST_FOR:           sprintf(wrapper->label, "FOR"); break;          // ← Removed (?)
	case AST_ELSE:          sprintf(wrapper->label, "ELSE"); break;
	...
	// Handle nodes with stmnts[] array (functions, returns, etc.)
	if (node->stmnt_cnt > 0) {
		wrapper->left = build_wrapper_tree(node->stmnts[0]);
		if (wrapper->left) wrapper->left->parent_dir = -1;

		if (node->stmnt_cnt > 1) {
			ASTWrapper *prev = wrapper->left;
			for (int i = 1; i < node->stmnt_cnt; i++) {
				if (node->stmnts[i]) {
					prev->right = build_wrapper_tree(node->stmnts[i]);
					if (prev->right) {
						prev->right->parent_dir = 1;
						prev = prev->right;
					}
				}
			}
		...
		// Regular nodes use left/right pointers
		wrapper->left = build_wrapper_tree(node->left);
		wrapper->right = build_wrapper_tree(node->right);
...
// ** NOTE -> LIMITATIONS: the tree structure still wrong somehow, it does not print silbling relationship, rather everything is parent and child and nested loop.
